...
from subgoals_for_crl import ContrastiveRLwithImaginedSubgoals, LaplacePolicy, ContrastiveSubgoalPolicy
...
def goal_to_obs(goal):
    ...
    """
    obs = torch.zeros((1, observation_shape), device=args.device, dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_name",           default="fetchPickandPlace_SGCRIS_R3")
    ...
    parser.add_argument("--temperature",        default=0.7 , type=float) #low => peaky; high => flatten (0,1)
    ...
    parser.add_argument("--load_model",         default=True, action='store_true', help="Flag to load model and continue training")
    parser.set_defaults(log_loss=True)
    args = parser.parse_args()
    print(args)
    
    env = gym.make(args.env_name, max_episode_steps=args.max_episode_length)
    env_test = gym.make(args.env_name, render_mode="human", max_episode_steps=args.max_episode_length) #used for range goals
    ...
    buffer = EpisodicBuffer(num_episodes=args.max_episodes, max_episode_length=args.max_episode_length, shapes=buffer_shapes)
    policy = ContinuousFeedForwardPolicy(size_in=observation_shape+goal_shape, size_out=action_shape).to(args.device)
    qf = ContrastiveQf(hidden_sizes=[256,256], representation_dim=args.repr_dim, action_dim=action_shape, goal_dim=goal_shape, obs_dim=observation_shape).to(args.device)
    if args.hlp_latent:
        subgoal_net = ContrastiveSubgoalPolicy(contrastive_qf=qf, state_dim=observation_shape, repr_dim=64, device=args.device)
    else:
        subgoal_net = LaplacePolicy(state_dim=observation_shape, goal_dim=goal_shape, device=args.device)
    if(args.use_subgoals):    
        crl = ContrastiveRLwithImaginedSubgoals(device=args.device, policy=policy, qf=qf, entropy_coefficient=None, target_entropy=0.0, bc_coef=0.05, subgoal_net=subgoal_net)
    else:
        crl = StableContrastiveRL(device=args.device, policy=policy, qf=qf, entropy_coefficient=None, target_entropy=0.0, bc_coef=0.05)
    ...
    for e in range(args.max_episodes):
        ...
        state = torch.tensor(obs_dict["observation"].reshape((1,-1)), device=args.device, dtype=torch.float32)
        success = 0.0
        ...
        for step in range(args.max_episode_length):
            with torch.no_grad():
                ...
                if total_steps < args.start_timesteps:
                    ...
                    next_obs_dict  = env.step(action.detach().cpu().numpy().reshape((-1,)))[0]

                next_state = torch.tensor(next_obs_dict["observation"].reshape((1,-1)), device=args.device, dtype=torch.float32)
                buffer.append({"observation" : state, "action" : action, "next_observation" : next_state, "desired_goal" : goal})
                
                if args.use_single_goal:
                    distance = np.linalg.norm(next_obs_dict["achieved_goal"] - single_goal_np)
                else:
                    distance = np.linalg.norm(next_obs_dict["achieved_goal"] - next_obs_dict["desired_goal"]) 
                state = next_state


            if total_steps >= args.start_timesteps and step % 16 == 0:
                if(args.use_subgoals):
                    if not args.use_single_goal: single_goal = 0
                    update_dict = crl.update(future_buffer, single_goal, batch_size=args.batch_size)
                else: 
                    update_dict = crl.update(future_buffer, batch_size=args.batch_size)
        ...
        if args.use_single_goal:
            ...
            state2 = torch.tensor(obs_dict2["observation"].reshape((1,-1)), device=args.device, dtype=torch.float32)
            success2 = 0.0
            for step2 in range(args.max_episode_length):
                with torch.no_grad():
                    ...
                    next_obs_dict2  = env_test.step(action2.detach().cpu().numpy().reshape((-1,)))[0]

                    next_state2 = torch.tensor(next_obs_dict2["observation"].reshape((1,-1)), device=args.device, dtype=torch.float32)
                    

                    distance2  = np.linalg.norm(next_obs_dict2["achieved_goal"] - next_obs_dict2["desired_goal"]) 
                    state2 = next_state2
        ...
        plotter.store_only(total_steps, (sum(successes)/100), (sum(successes2)/100)) #(sum(successes2)/100)
        
        if e % 1 == 0:
            print(f"[{e}, {total_steps}] {step+1} steps. Success rate: {sum(successes)} " + logger.print())
            logger.flush()
